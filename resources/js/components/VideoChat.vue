...
export default {
    ...
    methods: {
        ...
        initializeCallListeners() {
            console.log("initializeCallListeners is running");
            ...
            this.videoCallParams.channel.listen('.StartVideoChat', (event) => {
                console.log("Received StartVideoChat event:", event); // Log event data

                // Check if event.data.signal exists before proceeding
                if (event.data.signalData && event.data.signalData.sdp) {
                    const updatedSignal = {...event.data.signalData, sdp: `${event.data.signalData.sdp}\n`};

                    // Ensure we are receiving the 'incomingCall' type and it is from another user
                    if (event.data.type === 'incomingCall' && event.data.from !== this.authuserid) {
                        console.log("Incoming call from user:", event.data.from);
                        this.videoCallParams.receivingCall = true;
                        this.videoCallParams.caller = event.data.from;
                        this.videoCallParams.callerSignal = updatedSignal;
                    }
                } else {
                    console.error("No signal or SDP found in the event data:", event.data);
                }
            });

        },
        placeVideoCall: async function (id, name) {
            console.log("Attempting to place a call to user:", id, name);  // Debugging call initiation
            ...
            console.log("Media permissions granted, stream:", this.videoCallParams.stream);
            ...
            console.log("Peer connection initialized, waiting for signal...");

            this.videoCallParams.peer1.on("signal", (data) => {
                console.log("Signal received from peer1, sending signal to user:", data);  // Debugging signal data

                axios
                    .post("/admin/video/call-user", {
                        user_to_call: id,
                        signal_data: data,
            ...
            this.videoCallParams.peer1.on("stream", (stream) => {
                console.log("Streaming the call from peer1.");
            ...
            // Listen for call acceptance event from receiver
            this.videoCallParams.channel.listen(".StartVideoChat", (event) => {
                if (event.data.type === "callAccepted") {
                    if (event.data.signal.renegotiate) {
                        console.log("renegotating");
                    }
                    if (event.data.signal.sdp) {
                        this.videoCallParams.callAccepted = true;
                        const updatedSignal = {...event.data.signal, sdp: `${event.data.signal.sdp}\n`};
        ...
        async acceptCall() {
            ...
            this.videoCallParams.peer2.on("signal", (data) => {
                console.log("Signal from receiver:", data);
                axios
                    .post("/admin/video/accept-call", {
                        signal: data,
